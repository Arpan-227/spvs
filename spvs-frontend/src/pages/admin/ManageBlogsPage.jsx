  ...
  return (
    <div style={{maxWidth:'1100px', width:'100%', boxSizing:'border-box'}}>
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'24px',gap:'16px',flexWrap:'wrap'}}>
        <div>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(20px,4vw,26px)',fontWeight:'700',color:'#1C0A00',margin:'0 0 5px'}}>📰 Blogs & Updates</h1>
          <p style={{fontSize:'13px',color:'#7A4010',margin:0}}>Manage all school blog posts and announcements</p>
        </div>
        <button onClick={openAdd} style={{...s.btn,background:'linear-gradient(135deg,#E8761A,#F5B800)',color:'#fff',boxShadow:'0 6px 20px rgba(232,118,26,.3)',padding:'12px 20px',fontSize:'14px',whiteSpace:'nowrap'}}>
          + Add New Post
        </button>
      </div>
      ...
      <div className="mbp-stats">
        {[
          { label:'Total Posts', value:loading?'...':posts.length, icon:'📰', clr:'#E8761A' },
          { label:'Published',   value:loading?'...':pub,          icon:'✅', clr:'#22a35a' },
          { label:'Drafts',      value:loading?'...':draft,        icon:'📝', clr:'#C45F0A' },
        ].map(function(st){
          return (
      ...
      <div style={s.card}>
        {loading ? (
          <div style={{textAlign:'center',padding:'60px'}}>
            ...
            <div style={{color:'#B87832',fontSize:'14px'}}>Loading posts...</div>
          </div>
        ) : error ? (
                    ...
                    <tr key={p._id} style={{borderBottom:'1px solid rgba(232,118,26,.07)',background:i%2===0?'#FFFFFF':'#FFFDF8',transition:'background .15s'}}
                      ...
                      <td style={{padding:'8px 10px',width:'48px'}}>
                        {p.image
                          ...
                          : <div style={{width:'44px',height:'34px',borderRadius:'7px',background:'rgba(232,118,26,.08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'14px'}}>📰</div>
                        }
                      </td>
                      ...
                      <td style={{padding:'10px 12px'}}>
                        <span style={{padding:'3px 10px',borderRadius:'20px',fontSize:'11px',fontWeight:'800',background:cc+'15',color:cc,whiteSpace:'nowrap'}}>{p.category}</span>
                      </td>
                      <td style={{padding:'10px 12px'}}>
                        <button onClick={function(){toggleStatus(p)}}
                          style={{padding:'4px 12px',borderRadius:'20px',fontSize:'11px',fontWeight:'800',border:'none',cursor:'pointer',background:p.published?'rgba(34,163,90,.12)':'rgba(196,95,10,.12)',color:p.published?'#22a35a':'#C45F0A',transition:'all .2s',whiteSpace:'nowrap'}}>
                      ...
                      <td style={{padding:'10px 12px'}}>
                        <div style={{display:'flex',gap:'6px'}}>
                          ...
                          <button onClick={function(){openDel(p._id)}}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(28,10,0,.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'16px'}} onClick={function(e){if(e.target===e.currentTarget)closeModal()}}>
          <div style={{background:'#FFFDF8',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.2)',padding:'clamp(18px,4vw,30px)',width:'100%',maxWidth:'560px',maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(28,10,0,.2)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(16px,4vw,20px)',fontWeight:'700',color:'#1C0A00',margin:0}}>{modal==='add'?'+ Add New Post':'✏️ Edit Post'}</h2>
            ...
            <div style={{display:'flex',gap:'10px',justifyContent:'flex-end',marginTop:'20px',flexWrap:'wrap'}}>
              ...
              <button onClick={handleSave} disabled={saving} style={{...s.btn,background:'linear-gradient(135deg,#E8761A,#F5B800)',color:'#fff',boxShadow:'0 4px 14px rgba(232,118,26,.3)',opacity:saving?.7:1}}>
                {saving?'⏳ Saving...':modal==='add'?'+ Publish Post':'💾 Save Changes'}
              </button>
            </div>
          </div>
        </div>
