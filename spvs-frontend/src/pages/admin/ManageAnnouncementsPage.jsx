              ...
              {['All','Active','Inactive'].map(function(f){
                ...
                return (
                  <button key={f} onClick={function(){setFilter(f)}}
                    style={{padding:'7px 12px',borderRadius:'8px',border:'1.5px solid',borderColor:isA?'#E8761A':'rgba(232,118,26,.2)',background:isA?'linear-gradient(135deg,#E8761A,#F5B800)':'#FFF6EA',color:isA?'#fff':'#7A4010',fontSize:'12px',fontWeight:'700',cursor:'pointer',whiteSpace:'nowrap',transition:'all .15s'}}>
                    {f}
                  </button>
                )
              })}
      ...
      {loading ? (
        ...
        <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
          ...
          {visible.map(function(a){
            ...
            return (
              <div key={a._id} style={{...s.card,borderLeft:a.active?'4px solid #22a35a':'4px solid rgba(232,118,26,.2)',transition:'all .2s',padding:'16px 18px',opacity:a.active?1:.7}}
                ...
                <div className="man-item-inner">
                  <div className="man-item-info">
