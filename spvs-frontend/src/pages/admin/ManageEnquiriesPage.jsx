...
export default function ManageEnquiriesPage() {
  ...
  var [view,    setView]    = useState(null)   // view detail modal
  var [delId,   setDelId]   = useState(null)
  // local status map — stores admin-set statuses (backend has no status field, we manage locally + note)
    ...
    <div style={{maxWidth:'1100px'}}>
      <style>{`
        ...
        @media (max-width:700px) {
          .meq-status-cards { grid-template-columns:repeat(3,1fr); }
        }
        @media (max-width:480px) {
          .meq-status-cards { grid-template-columns:repeat(2,1fr); }
          .meq-filters      { flex-direction:column; align-items:stretch; }
        }
      `}</style>
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'24px',gap:'16px',flexWrap:'wrap'}}>
        <div>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(20px,4vw,26px)',fontWeight:'700',color:'#1C0A00',margin:'0 0 5px'}}>📋 Enquiries</h1>
          <p style={{fontSize:'13px',color:'#7A4010',margin:0}}>
            All contact and admission enquiries from the website
            {unread > 0 && <span style={{marginLeft:'8px',padding:'2px 10px',borderRadius:'20px',background:'#E8761A',color:'#fff',fontSize:'11px',fontWeight:'800'}}>{unread} unread</span>}
          </p>
        </div>
        <button onClick={fetchEnquiries} style={{...s.btn,background:'#FFF6EA',color:'#7A4010',border:'1.5px solid rgba(232,118,26,.2)',padding:'10px 18px'}}>🔄 Refresh</button>
      </div>
      ...
      <div style={{...s.card,marginBottom:'16px',padding:'14px 16px'}}>
        <div className="meq-filters">
          <input value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="🔍  Search by name or phone..." style={{...s.inp,width:'100%',maxWidth:'240px',padding:'9px 13px'}} onFocus={inf} onBlur={inb} />
      ...
      <div style={s.card}>
        {loading ? (
          <div style={{textAlign:'center',padding:'60px'}}>
            ...
            <div style={{color:'#B87832',fontSize:'14px'}}>Loading enquiries...</div>
          </div>
        ) : (
                      ...
                      <td style={{padding:'10px 12px'}}>
                        <div style={{display:'flex',gap:'5px'}}>
                          <button onClick={function(){setView(e);if(!e.read)markRead(e._id)}} title="View"
                            ...
                            onMouseLeave={function(el){el.currentTarget.style.background='#FFF6EA';el.currentTarget.style.borderColor='rgba(232,118,26,.2)'}}>👁️</button>
                          <button onClick={function(){setDelId(e._id);setModal('delete')}} title="Delete"
                            style={{width:'30px',height:'30px',borderRadius:'8px',border:'1.5px solid rgba(220,38,38,.15)',background:'rgba(254,242,242,.6)',cursor:'pointer',fontSize:'13px',display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}
                            onMouseEnter={function(el){el.currentTarget.style.background='#dc2626'}}
                            onMouseLeave={function(el){el.currentTarget.style.background='rgba(254,242,242,.6)'}}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  )
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(28,10,0,.45)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'16px'}} onClick={function(e){if(e.target===e.currentTarget)setView(null)}}>
          <div style={{background:'#FFFDF8',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.2)',padding:'clamp(18px,4vw,30px)',width:'100%',maxWidth:'500px',maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(28,10,0,.2)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:'18px',fontWeight:'700',color:'#1C0A00',margin:0}}>📋 Enquiry Detail</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {[
                ...
                ['📞 Phone',   view.phone],
                ['📧 Email',   view.email||'—'],
                ['🏷️ Type',    view.type||'General'],
                ['📅 Date',    fmtDate(view.createdAt)],
              ].map(function(row){
                return (
                  <div key={row[0]} style={{display:'flex',gap:'12px',padding:'10px 14px',borderRadius:'10px',background:'#FFF6EA',border:'1px solid rgba(232,118,26,.1)'}}>
                    <span style={{fontSize:'12px',fontWeight:'800',color:'#B87832',minWidth:'80px'}}>{row[0]}</span>
              ...
              {view.message && (
                <div style={{padding:'12px 14px',borderRadius:'10px',background:'#FFF6EA',border:'1px solid rgba(232,118,26,.1)'}}>
                  <div style={{fontSize:'12px',fontWeight:'800',color:'#B87832',marginBottom:'6px'}}>💬 Message</div>
