...
var TABS = [
  { key: 'schoolInfo',     label: 'School Info',    icon: '🏫' },
    ...
    <div style={{ maxWidth: '960px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          style={{ padding: '10px 24px', borderRadius: '10px', border: 'none', background: saveBg, color: '#fff', fontSize: '13px', fontWeight: '800', cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, transition: 'all .25s' }}
        >
          {saveText}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '20px', background: '#fff', padding: '5px', borderRadius: '12px', border: '1.5px solid rgba(232,118,26,.12)', width: 'fit-content', flexWrap: 'wrap' }}>
        {TABS.map(function(t) {
          ...
          var tabStyle = {
            ...
            fontSize: '12px', whiteSpace: 'nowrap', transition: 'all .18s',
            fontWeight: isActive ? '800' : '600',
            background: isActive ? 'linear-gradient(135deg,#E8761A,#F5B800)' : 'transparent',
            color: isActive ? '#fff' : '#B87832',
          }
          return (
            <button key={t.key} onClick={function() { setTab(t.key) }} style={tabStyle}>
              {t.icon} {t.label}
            </button>
          )
        })}
      ...
      <div style={{ ...s.card, padding: '24px' }}>
        ...
        {tab === 'staff' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', paddingBottom: '14px', borderBottom: '1.5px solid rgba(232,118,26,.1)' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '9px', background: 'rgba(59,130,246,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px' }}>👨‍🏫</div>
              <div style={s.h2}>Staff Information</div>
            </div>
            {renderTable('staff', data.staff)}
          </div>
        )}
        ...
        {tab === 'transport' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', paddingBottom: '14px', borderBottom: '1.5px solid rgba(232,118,26,.1)' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '9px', background: 'rgba(245,158,11,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px' }}>🚌</div>
              <div style={s.h2}>Transport Details</div>
            </div>
            {renderTable('transport', data.transport)}
          </div>
        )}
