...
import { FaImages, FaTags, FaEye, FaCheckCircle, FaTimesCircle, FaPencilAlt, FaTrash, FaSpinner, FaCamera, FaFolder, FaSearch } from 'react-icons/fa'
...
export default function ManageGalleryPage() {
  ...
  var STATS = [
    { label:'Total Photos', value:loading?'...':items.length,  icon:<FaCamera size={18} color="#E8761A"/>,  clr:'#E8761A' },
    { label:'Categories',   value:CATS.length-1,               icon:<FaTags size={18} color="#6C3FC5"/>,    clr:'#6C3FC5' },
    { label:'Showing',      value:loading?'...':visible.length, icon:<FaEye size={18} color="#22a35a"/>,    clr:'#22a35a' },
  ]
  ...
  return (
    <div style={{maxWidth:'1100px',width:'100%',boxSizing:'border-box'}}>
      ...
      {toast && <div style={{position:'fixed',top:'20px',right:'20px',zIndex:2000,padding:'12px 20px',borderRadius:'12px',background:toast.type==='error'?'#dc2626':'#22a35a',color:'#fff',fontFamily:"'DM Sans',sans-serif",fontSize:'13px',fontWeight:'700',boxShadow:'0 8px 24px rgba(0,0,0,.2)',animation:'slideIn .3s ease',display:'inline-flex',alignItems:'center',gap:'7px'}}>{toast.type==='error'?<FaTimesCircle size={13}/>:<FaCheckCircle size={13}/>} {toast.msg}</div>}
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'24px',gap:'16px',flexWrap:'wrap'}}>
        <div>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(20px,4vw,26px)',fontWeight:'700',color:'#1C0A00',margin:'0 0 5px',display:'inline-flex',alignItems:'center',gap:'10px'}}>
            <FaImages size={22} color="#E8761A"/> Gallery Management
          </h1>
      ...
      <div style={{...s.card,marginBottom:'16px',padding:'14px 16px'}}>
        <div className="mgp-filters">
          <div style={{position:'relative'}}>
            ...
            <input className="mgp-search" value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="Search photos..."
              style={{...s.inp,maxWidth:'200px',padding:'9px 13px 9px 32px'}} onFocus={inp_focus} onBlur={inp_blur} />
          </div>
      ...
      {loading ? (
        <div style={{...s.card,textAlign:'center',padding:'60px',display:'flex',alignItems:'center',justifyContent:'center',gap:'10px'}}><FaSpinner size={20} color="#E8761A" style={{animation:'spin .8s linear infinite'}}/> Loading photos...</div>
      ) : error ? (
            ...
            return (
              <div key={a._id} style={{...s.card,padding:0,overflow:'hidden',transition:'all .25s'}} onMouseEnter={function(e){e.currentTarget.style.transform='translateY(-3px)';e.currentTarget.style.boxShadow='0 10px 28px rgba(232,118,26,.15)'}} onMouseLeave={function(e){e.currentTarget.style.transform='none';e.currentTarget.style.boxShadow='0 4px 16px rgba(232,118,26,.06)'}}>
                <div style={{height:'120px',overflow:'hidden',position:'relative'}}>
                  {a.image?<img src={a.image} alt={a.title} style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>:<div style={{width:'100%',height:'100%',background:cc+'18',display:'flex',alignItems:'center',justifyContent:'center'}}><FaImages size={40} color={cc}/></div>}
                ...
                <div style={{padding:'10px 12px'}}>
                  ...
                  <div style={{display:'flex',gap:'6px'}}>
                    <button onClick={function(){openEdit(a)}} style={{flex:1,padding:'6px',borderRadius:'7px',border:'1.5px solid rgba(232,118,26,.2)',background:'#FFF6EA',color:'#E8761A',fontSize:'11px',fontWeight:'700',cursor:'pointer',transition:'all .15s',display:'inline-flex',alignItems:'center',justifyContent:'center',gap:'4px'}} onMouseEnter={function(e){e.currentTarget.style.background='#E8761A';e.currentTarget.style.color='#fff'}} onMouseLeave={function(e){e.currentTarget.style.background='#FFF6EA';e.currentTarget.style.color='#E8761A'}}><FaPencilAlt size={11}/> Edit</button>
                    <button onClick={function(){openDel(a._id)}} style={{width:'30px',borderRadius:'7px',border:'1.5px solid rgba(220,38,38,.15)',background:'rgba(254,242,242,.6)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}} onMouseEnter={function(e){e.currentTarget.style.background='#dc2626'}} onMouseLeave={function(e){e.currentTarget.style.background='rgba(254,242,242,.6)'}}><FaTrash size={12} color="#dc2626"/></button>
                  </div>
                </div>
              </div>
            )
                ...
                {visible.map(function(a,i){
                  ...
                  return (
                    <tr key={a._id} style={{borderBottom:'1px solid rgba(232,118,26,.07)',background:i%2===0?'#FFFFFF':'#FFFDF8',transition:'background .15s'}} onMouseEnter={function(e){e.currentTarget.style.background='#FFF6EA'}} onMouseLeave={function(e){e.currentTarget.style.background=i%2===0?'#FFFFFF':'#FFFDF8'}}>
                      <td style={{padding:'8px 12px',width:'60px'}}>{a.image?<img src={a.image} alt="" style={{width:'52px',height:'40px',objectFit:'cover',borderRadius:'8px',display:'block'}}/>:<div style={{width:'52px',height:'40px',borderRadius:'8px',background:'rgba(232,118,26,.08)',display:'flex',alignItems:'center',justifyContent:'center'}}><FaImages size={18} color="#E8761A"/></div>}</td>
                      ...
                      <td style={{padding:'10px 12px'}}><div style={{display:'flex',gap:'6px'}}><button onClick={function(){openEdit(a)}} style={{width:'32px',height:'32px',borderRadius:'8px',border:'1.5px solid rgba(232,118,26,.2)',background:'#FFF6EA',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}} onMouseEnter={function(e){e.currentTarget.style.background='#E8761A'}} onMouseLeave={function(e){e.currentTarget.style.background='#FFF6EA'}}><FaPencilAlt size={13} color="#7A4010"/></button><button onClick={function(){openDel(a._id)}} style={{width:'32px',height:'32px',borderRadius:'8px',border:'1.5px solid rgba(220,38,38,.15)',background:'rgba(254,242,242,.6)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}} onMouseEnter={function(e){e.currentTarget.style.background='#dc2626'}} onMouseLeave={function(e){e.currentTarget.style.background='rgba(254,242,242,.6)'}}><FaTrash size={13} color="#dc2626"/></button></div></td>
                    </tr>
                  )
                })}
              </tbody>
          ...
          <div style={{background:'#FFFDF8',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.2)',padding:'clamp(18px,4vw,30px)',width:'100%',maxWidth:'500px',maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(28,10,0,.2)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(16px,4vw,20px)',fontWeight:'700',color:'#1C0A00',margin:0,display:'inline-flex',alignItems:'center',gap:'8px'}}>
                {modal==='add' ? <><FaCamera size={16} color="#E8761A"/> Upload Photo</> : <><FaPencilAlt size={16} color="#E8761A"/> Edit Photo</>}
              </h2>
              ...
              <div>
                ...
                <div style={{border:'2px dashed rgba(232,118,26,.25)',borderRadius:'12px',padding:'14px',background:'rgba(232,118,26,.03)'}}>
                  {(imgFile||current.image)?(
                    ...
                    <div style={{textAlign:'center',padding:'20px 0',marginBottom:'8px'}}>
                      <FaCamera size={36} color="#B87832" style={{marginBottom:'6px'}}/>
                      <div style={{fontSize:'12.5px',color:'#B87832',fontWeight:'600'}}>Upload a photo from your device</div>
                      <div style={{fontSize:'11px',color:'rgba(184,120,50,.6)',marginTop:'3px'}}>JPG, PNG, WEBP — Max 5MB</div>
                    </div>
                  )}
                  <label style={{display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',padding:'9px 14px',borderRadius:'9px',border:'1.5px solid rgba(232,118,26,.25)',background:'#fff',cursor:'pointer',fontSize:'13px',fontWeight:'700',color:'#E8761A',marginBottom:'8px'}}>
                    <FaFolder size={13}/> Upload from Device<input type="file" accept="image/*" style={{display:'none'}} onChange={function(e){var f=e.target.files&&e.target.files[0];if(f){setImgFile(f);setCurrent(function(p){return{...p,image:''}})}}} />
                  </label>
            ...
            <div style={{display:'flex',gap:'10px',justifyContent:'flex-end',marginTop:'20px',flexWrap:'wrap'}}>
              ...
              <button onClick={handleSave} disabled={saving} style={{...s.btn,background:'linear-gradient(135deg,#E8761A,#F5B800)',color:'#fff',boxShadow:'0 4px 14px rgba(232,118,26,.3)',opacity:saving?.7:1,display:'inline-flex',alignItems:'center',gap:'7px'}}>
                {saving?<><FaSpinner size={13} style={{animation:'spin .8s linear infinite'}}/> Saving...</>:modal==='add'?'+ Upload Photo':'Save Changes'}
              </button>
            </div>
          </div>
        </div>
