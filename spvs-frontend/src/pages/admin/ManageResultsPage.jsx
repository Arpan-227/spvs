...
// ✅ Grade based on raw sum out of 200
...
// ✅ Total marks = sum of marksObtained (each out of 100)
function computeTotals(subjects) {
  var totalMax      = subjects.length * 100
  var totalObtained = subjects.reduce(function(sum, sub) {
    return sum + (Number(sub.marksObtained) || 0)
  }, 0)
  var pct = totalMax > 0
...
export default function ManageResultsPage() {
  ...
  function openAdd() {
    setCurrent(makeEmptyResult('Class VI'))
    setEditId(null); setModal('form'); setActiveTab('info'); setSaved(false); setError('')
  }

  function openEdit(r) {
    setCurrent({...r})
    setEditId(r._id); setModal('form'); setActiveTab('info'); setSaved(false); setError('')
  }

  function closeModal() {
    setModal(null); setCurrent(makeEmptyResult('Class VI'))
    setEditId(null); setSaved(false); setError('')
  }
  ...
  function handleInfo(e) {
    ...
    setCurrent(function(p) {
      ...
      if (k === 'class') {
        ...
        n.totalMaxMarks = totals.totalMaxMarks
        n.totalMarks    = totals.totalMarks
        n.percentage    = totals.percentage
      }
      return n
    })
  }

  // ✅ Key function — rawSum/2 = marks obtained, grade from rawSum
  function handleSubject(idx, field, val) {
    setCurrent(function(p) {
      var subs = p.subjects.map(function(s, i) {
        ...
        // Raw sum out of 200
        var rawSum = Number(updated.t1BestPT||0)       +
                     Number(updated.t1NoteBook||0)      +
                     Number(updated.t1SubjectEnrich||0) +
                     Number(updated.t1HalfYearly||0)    +
                     Number(updated.t2BestPT||0)        +
                     Number(updated.t2NoteBook||0)      +
                     Number(updated.t2SubjectEnrich||0) +
                     Number(updated.t2Annual||0)

        // ✅ Displayed marks = rawSum / 2 (out of 100)
        updated.marksObtained = Math.round(rawSum / 2)
        updated.maxMarks      = 100

        // ✅ Grade based on 200-scale raw sum
        updated.grade = calcGrade(rawSum)
  ...
  function handleCoScholastic(idx, val) {
    setCurrent(function(p) {
      var arr = p.coScholastic.map(function(c, i) { return i === idx ? {...c, grade: val} : c })
      return {...p, coScholastic: arr}
    })
  }

  function handleCharacter(idx, val) {
    setCurrent(function(p) {
      var arr = p.characterValues.map(function(c, i) { return i === idx ? {...c, grade: val} : c })
      return {...p, characterValues: arr}
    })
  }
  ...
  async function handleDelete() {
    try {
      await resultAPI.delete(delId)
      setResults(function(p) { return p.filter(function(r) { return r._id !== delId }) })
      closeModal()
  ...
  async function toggleStatus(r) {
    ...
    try {
      var res = await resultAPI.update(r._id, {...r, status: newStatus})
      setResults(function(p) { return p.map(function(x) { return x._id === r._id ? res.data : x }) })
    } catch (err) {}
  }
  ...
  return (
    <div style={{maxWidth:'1100px'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))', gap:'12px', marginBottom:'20px'}}>
        {[
          {label:'Total',     value:results.length,                                                    icon:'📊', clr:'#E8761A'},
          {label:'Published', value:results.filter(function(r){return r.status==='Published'}).length, icon:'✅', clr:'#22a35a'},
          {label:'Drafts',    value:results.filter(function(r){return r.status==='Draft'}).length,     icon:'📝', clr:'#6C3FC5'},
        ].map(function(st) {
          return (
        ...
        {loading ? (
          ...
          <div style={{overflowX:'auto'}}>
            <table style={{width:'100%', borderCollapse:'collapse'}}>
              ...
              <tbody>
                {visible.length === 0 && (
                  <tr><td colSpan={9} style={{padding:'40px', textAlign:'center', color:'#B87832'}}>No results found</td></tr>
                )}
          ...
          <div style={{background:'#FFFDF8', borderRadius:'20px', border:'1.5px solid rgba(232,118,26,.2)', width:'100%', maxWidth:'860px', maxHeight:'92vh', overflowY:'auto', boxShadow:'0 24px 64px rgba(28,10,0,.25)'}}>
            ...
            <div style={{background:'linear-gradient(135deg,#E8761A,#F5B800)', padding:'18px 24px', borderRadius:'18px 18px 0 0', display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, zIndex:10}}>
              <div style={{fontFamily:"'Playfair Display',serif", fontSize:'18px', fontWeight:'700', color:'#1C0A00'}}>
                {editId ? '✏️ Edit Result' : '+ Add Result'}
              </div>
            ...
            <div style={{padding:'22px 24px'}}>
              ...
              {error && (
                <div style={{marginBottom:'14px', padding:'10px 14px', borderRadius:'10px', background:'rgba(220,38,38,.08)', border:'1px solid rgba(220,38,38,.2)', color:'#dc2626', fontSize:'13px', fontWeight:'600'}}>
                  ❌ {error}
                </div>
              )}
              {saved && (
                <div style={{marginBottom:'14px', padding:'10px 14px', borderRadius:'10px', background:'rgba(34,163,90,.08)', border:'1px solid rgba(34,163,90,.2)', color:'#22a35a', fontSize:'13px', fontWeight:'600'}}>
                  ✅ Saved successfully!
                </div>
              )}
              ...
              <div style={{display:'flex', gap:'4px', background:'#fff', padding:'4px', borderRadius:'10px', border:'1.5px solid rgba(232,118,26,.12)', width:'fit-content', marginBottom:'20px', flexWrap:'wrap'}}>
                {[['info','📋 Student Info'],['marks','📊 Marks'],['activities','🎯 Activities'],['meta','⚙️ Meta']].map(function(t) {
                  ...
                  {[
                    ...
                    return (
                      <div key={f.name}>
                        ...
                        <input name={f.name} type={f.type||'text'} value={current[f.name]||''} onChange={handleInfo} placeholder={f.placeholder||''} style={s.inp} />
                      </div>
                    )
                  })}
                  <div>
                    <label style={s.label}>Class</label>
                    <select name="class" value={current.class} onChange={handleInfo} style={s.inp}>
                      {CLASSES.map(function(c){ return <option key={c}>{c}</option> })}
                    </select>
                  </div>
                  <div>
                    <label style={s.label}>Section</label>
                    <select name="section" value={current.section} onChange={handleInfo} style={s.inp}>
                      {SECTIONS.map(function(c){ return <option key={c}>{c}</option> })}
                    </select>
                  </div>
                  <div>
                    <label style={s.label}>Session</label>
                    <select name="session" value={current.session} onChange={handleInfo} style={s.inp}>
                      {SESSIONS.map(function(c){ return <option key={c}>{c}</option> })}
                    </select>
                  </div>
                  <div>
                    <label style={s.label}>Status</label>
                    <select name="status" value={current.status} onChange={handleInfo} style={s.inp}>
                ...
                <div>
                  ...
                  <div style={{background:'rgba(232,118,26,.05)', border:'1.5px solid rgba(232,118,26,.15)', borderRadius:'12px', padding:'12px 16px', marginBottom:'16px'}}>
                    ...
                    <div style={{display:'flex', flexWrap:'wrap', gap:'6px'}}>
                      {[['181–200','A1'],['161–180','A2'],['141–160','B1'],['121–140','B2'],['101–120','C1'],['81–100','C2'],['66–80','D'],['≤65','E']].map(function(g){
                        return (
                          <span key={g[1]} style={{padding:'3px 10px', borderRadius:'20px', fontSize:'11px', fontWeight:'800', background:'rgba(232,118,26,.1)', color:'#C45F0A'}}>
                            {g[0]} → {g[1]}
                          </span>
                        )
                      })}
                    </div>
                    <div style={{fontSize:'11.5px', color:'#7A4010', marginTop:'8px', fontWeight:'600'}}>
                      ✅ Marks Obtained = Raw Sum ÷ 2 (displayed out of 100)
                    </div>
                  </div>
                    ...
                    <table style={{width:'100%', borderCollapse:'collapse', fontSize:'12px', minWidth:'800px'}}>
                      <thead>
                        <tr style={{background:'#FFF6EA'}}>
                          ...
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#22a35a', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T1 PT<br/><span style={{fontWeight:'600'}}>(10)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#22a35a', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T1 NB<br/><span style={{fontWeight:'600'}}>(5)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#22a35a', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T1 SE<br/><span style={{fontWeight:'600'}}>(5)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#22a35a', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>Half Yearly<br/><span style={{fontWeight:'600'}}>(80)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#6C3FC5', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T2 PT<br/><span style={{fontWeight:'600'}}>(10)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#6C3FC5', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T2 NB<br/><span style={{fontWeight:'600'}}>(5)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#6C3FC5', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>T2 SE<br/><span style={{fontWeight:'600'}}>(5)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#6C3FC5', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>Annual<br/><span style={{fontWeight:'600'}}>(80)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#E8761A', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>Max<br/><span style={{fontWeight:'600'}}>(100)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#E8761A', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>Obtained<br/><span style={{fontWeight:'600'}}>(÷2)</span></th>
                          <th style={{padding:'8px', textAlign:'center', fontWeight:'800', color:'#C45F0A', borderBottom:'2px solid rgba(232,118,26,.15)', fontSize:'11px'}}>Grade</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            <tr key={idx} style={{borderBottom:'1px solid rgba(232,118,26,.06)', background:rowBg}}>
                              ...
                              {['t1BestPT','t1NoteBook','t1SubjectEnrich','t1HalfYearly','t2BestPT','t2NoteBook','t2SubjectEnrich','t2Annual'].map(function(field) {
                                return (
                                  <td key={field} style={{padding:'3px'}}>
                                    <input type="number" min="0" value={sub[field]||0}
                                      onChange={function(e){handleSubject(idx, field, e.target.value)}}
                                      style={{width:'50px', padding:'5px 4px', borderRadius:'6px', border:'1.5px solid rgba(232,118,26,.2)', background:'#FFFDF8', color:'#1C0A00', fontSize:'12px', textAlign:'center', outline:'none', fontFamily:"'DM Sans',sans-serif"}} />
                                  </td>
                                )
                              })}
                              ...
                              <td style={{padding:'6px 4px', textAlign:'center'}}>
                                <span style={{padding:'3px 9px', borderRadius:'20px', fontSize:'11px', fontWeight:'800', background:'rgba(232,118,26,.1)', color:'#C45F0A'}}>{sub.grade}</span>
                              </td>
                            </tr>
                          )
                        })}
                  ...
                  <div style={{marginTop:'16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px'}}>
                    <div>
                      ...
                      <input name="rank" value={current.rank||''} onChange={handleInfo} placeholder="e.g. 8" style={s.inp} />
                    </div>
                    <div>
                      <label style={s.label}>Result</label>
                      <select name="result" value={current.result} onChange={handleInfo} style={s.inp}>
                        <option>Pass</option><option>Fail</option><option>Promoted</option>
                      </select>
                    </div>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div style={{background:'rgba(232,118,26,.04)', borderRadius:'12px', padding:'14px', border:'1.5px solid rgba(232,118,26,.1)'}}>
                      {current.coScholastic.map(function(c, idx) {
                        var rowBg = idx%2===0 ? 'transparent' : 'rgba(232,118,26,.04)'
                        return (
                          <div key={idx} style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 10px', borderRadius:'8px', marginBottom:'4px', background:rowBg}}>
                            <span style={{fontSize:'13px', fontWeight:'600', color:'#1C0A00'}}>{c.activity}</span>
                            <select value={c.grade} onChange={function(e){handleCoScholastic(idx, e.target.value)}}
                              style={{padding:'5px 10px', borderRadius:'7px', border:'1.5px solid rgba(232,118,26,.2)', background:'#FFFDF8', fontSize:'13px', fontWeight:'800', color:'#E8761A', outline:'none', cursor:'pointer'}}>
                  ...
                  <div>
                    ...
                    <div style={{background:'rgba(108,63,197,.04)', borderRadius:'12px', padding:'14px', border:'1.5px solid rgba(108,63,197,.1)'}}>
                      {current.characterValues.map(function(c, idx) {
                        var rowBg = idx%2===0 ? 'transparent' : 'rgba(108,63,197,.04)'
                        return (
                          <div key={idx} style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 10px', borderRadius:'8px', marginBottom:'4px', background:rowBg}}>
                            <span style={{fontSize:'13px', fontWeight:'600', color:'#1C0A00'}}>{c.activity}</span>
                            <select value={c.grade} onChange={function(e){handleCharacter(idx, e.target.value)}}
                              style={{padding:'5px 10px', borderRadius:'7px', border:'1.5px solid rgba(108,63,197,.2)', background:'#FFFDF8', fontSize:'13px', fontWeight:'800', color:'#6C3FC5', outline:'none', cursor:'pointer'}}>
                              {GRADES_5.map(function(g){ return <option key={g}>{g}</option> })}
                            </select>
                          </div>
                        )
              ...
              {activeTab === 'meta' && (
                <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px'}}>
                  <div style={{gridColumn:'1/-1'}}>
                    <label style={s.label}>Class Teacher's Remarks</label>
                    <textarea name="teacherRemarks" value={current.teacherRemarks||''} onChange={handleInfo} rows={3}
                      style={{...s.inp, resize:'vertical', lineHeight:'1.6'}} placeholder="e.g. Excellent performance, keep it up!" />
                  </div>
                  <div>
                    <label style={s.label}>Date of Issue</label>
                    <input name="date" type="date" value={current.date||''} onChange={handleInfo} style={s.inp} />
                  </div>
                  <div>
                    <label style={s.label}>Status</label>
                    <select name="status" value={current.status} onChange={handleInfo} style={s.inp}>
            ...
            <div style={{padding:'16px 24px 20px', borderTop:'1px solid rgba(232,118,26,.1)', display:'flex', gap:'10px', justifyContent:'flex-end', position:'sticky', bottom:0, background:'#FFFDF8', borderRadius:'0 0 20px 20px'}}>
              ...
              <button onClick={handleSave} disabled={saving}
                style={{...s.btn, background:saving?'#ccc':'linear-gradient(135deg,#E8761A,#F5B800)', color:'#fff', boxShadow:'0 4px 14px rgba(232,118,26,.3)', opacity:saving?0.7:1, padding:'10px 28px'}}>
                {saving ? '⏳ Saving...' : editId ? '💾 Save Changes' : '+ Add Result'}
              </button>
            </div>
          </div>
        </div>
