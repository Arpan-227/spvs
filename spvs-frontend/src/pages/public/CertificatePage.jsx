...
/* ══════════════════════════════════════════
   CALENDAR PICKER — timezone bug fixed
══════════════════════════════════════════ */
...
/* ══════════════════════════════════════════
   ...
   (green tick, summary card, download btn)
══════════════════════════════════════════ */
function SuccessScreen({ data, certId, onDownload, onViewFull, onBack }) {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ width:'100%', maxWidth:'480px', textAlign:'center', animation:'fadeUp .5s ease both' }}>
          ...
          <div style={{
            background:'linear-gradient(135deg,#FFF9F0,#FEF3DC)',
            borderRadius:'20px',
            ...
            {rows.map(function(row, i){
              return (
                <div key={row.l} style={{
                  ...
                  padding:'9px 0',
                  borderBottom: i < rows.length-1 ? '1px dashed rgba(232,118,26,.15)' : 'none',
                  fontSize:'13.5px'
                }}>
                  <span style={{ color:'#B87832', fontWeight:'700' }}>{row.l}</span>
                  <span style={{
                    color: row.l==='Result' ? (row.v==='Pass'||row.v==='Promoted' ? '#166534' : '#dc2626') : '#1C0A00',
                    fontWeight:'700'
                  }}>{row.v}</span>
                </div>
              )
          ...
          <div style={{ display:'flex', gap:'12px', justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={onDownload}
              style={{
                ...
                boxShadow:'0 8px 28px rgba(232,118,26,.32)',
                fontFamily:"'DM Sans',sans-serif",
                transition:'all .22s'
              }}
            ...
            <button onClick={onBack}
              style={{
                ...
                background:'transparent', color:'#7A4010',
                fontWeight:'700', fontSize:'14px',
                border:'1.5px solid rgba(232,118,26,.28)', cursor:'pointer',
...
/* ══════════════════════════════════════════
   VERIFY SCREEN
   loader (3s) → success card OR full report OR error
══════════════════════════════════════════ */
function VerifyScreen({ certId, regNo, dob, onBack }) {
  ...
  useEffect(function(){
    /* Use tcAPI for TC, resultAPI for results */
  ...
  /* ── TC success — show TcCertificate component ── */
  if (resultData && certId === 'tc') {
    return <TcCertificate data={resultData} onBack={onBack} />
  }
  ...
  /* ── Results: show success summary screen ── */
  if (resultData && certId === 'results') {
    return (
      <SuccessScreen
        data={resultData}
        certId={certId}
        onDownload={function(){ setShowFull(true) }}
        onBack={onBack}
      />
    )
  }
    ...
    <div style={{minHeight:'62vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'48px 24px',fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:'center',maxWidth:'460px'}}>
        ...
        <div style={{display:'flex',gap:'12px',justifyContent:'center',flexWrap:'wrap'}}>
          ...
          <a href="tel:+919198783830" style={{display:'inline-flex',alignItems:'center',gap:'7px',padding:'13px 22px',borderRadius:'13px',background:'transparent',color:'#E8761A',fontWeight:'700',fontSize:'14px',border:'1.5px solid rgba(232,118,26,.28)',textDecoration:'none',fontFamily:"'DM Sans',sans-serif"}}>📞 Call School</a>
        </div>
      </div>
    </div>
  )
          ...
          <div style={{maxWidth:'620px',margin:'0 auto',position:'relative',zIndex:1,animation:'heroFadeUp .7s ease both'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:'8px',justifyContent:'center'}}>
              {CERTS.map(function(c){
                return <div key={c.id} style={{display:'inline-flex',alignItems:'center',gap:'6px',padding:'6px 13px',borderRadius:'50px',background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',fontSize:'12px',fontWeight:'600',color:'rgba(255,220,150,.7)'}}><span>{c.icon}</span>{c.label}</div>
              })}
            </div>
          </div>
        </section>
        ...
        <section style={{padding:'60px 24px 80px',display:'flex',justifyContent:'center'}}>
          <div style={{width:'100%',maxWidth:'560px'}}>
            ...
            <div style={{marginTop:'22px',display:'flex',gap:'10px',justifyContent:'center',flexWrap:'wrap'}}>
              {[{href:'tel:+919198783830',icon:'📞',text:'Call School Office'},{href:'mailto:spvbrh@gmail.com',icon:'✉️',text:'Email Us'}].map(function(btn){
                return (
