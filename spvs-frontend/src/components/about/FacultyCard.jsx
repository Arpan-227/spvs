...
export default function FacultyCard({ name, designation, subject, qualification, experience, phone, department }) {
  var roleKey = designation && designation.includes('Principal') ? designation
              : designation && designation.includes('P.G.T')    ? 'P.G.T'
              : designation && designation.includes('T.G.T')    ? 'T.G.T'
              : designation && designation.includes('Primary')  ? 'Primary'
              : 'Office'
  ...
  return (
    <div style={{background:'#FFFFFF',borderRadius:'18px',border:'1.5px solid rgba(232,118,26,.12)',padding:'24px',boxShadow:'0 4px 16px rgba(232,118,26,.06)',transition:'all .25s',display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center'}}
      ...
      {/* Experience */}
      {experience && (
        <div style={{fontSize:'11.5px',color:'#7A4010',fontWeight:'700',marginBottom:phone?'6px':'0',padding:'3px 10px',borderRadius:'8px',background:'rgba(232,118,26,.08)',border:'1px solid rgba(232,118,26,.12)'}}>
          {experience}
        </div>
      )}
      ...
      {phone && (
        <div style={{display:'flex',alignItems:'center',gap:'5px',marginTop:'8px',fontSize:'12px',color:'#7A4010',fontWeight:'600'}}>
          <FaPhone size={11} color="#E8761A"/> {phone}
        </div>
      )}
    </div>
  )
