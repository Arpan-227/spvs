      ...
      <div className="s-cont">
        ...
        <div className="contact-grid">
          <div className="ct-info">
            {[
              { ic:'📍', lbl:'Address',      val: address },
              { ic:'📞', lbl:'Phone',        val:<><a href={'tel:+91'+phone1}>+91 {phone1}</a><br/><a href={'tel:+91'+phone2}>+91 {phone2} (Principal)</a></> },
              { ic:'✉️', lbl:'Email',        val:<a href={'mailto:'+email}>{email}</a> },
              { ic:'🌐', lbl:'Website',      val:'www.santpathikvidyalaya.org' },
              { ic:'📋', lbl:'CBSE Details', val:'Affiliation No. ' + affNo + ' · School No. ' + schoolNo + ' · UDISE: ' + udise },
          ...
          <div className="form-box rv" style={{transitionDelay:'.2s'}}>
            ...
            {error && (
              <div style={{background:'rgba(239,68,68,.06)',border:'1.5px solid rgba(239,68,68,.2)',borderRadius:'10px',padding:'10px 14px',marginBottom:'12px',fontSize:'12.5px',color:'#dc2626',fontWeight:'600'}}>⚠️ {error}</div>
            )}
            {sent ? (
              ...
              <form onSubmit={handleSubmit}>
                ...
                <button className="fsub" type="submit" disabled={loading} style={{opacity:loading?.7:1,cursor:loading?'wait':'pointer'}}>
