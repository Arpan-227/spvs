...
function Field({ label, name, value, onChange, placeholder, type, full }) {
  return (
    <div style={full ? { gridColumn:'1/-1' } : {}}>
      ...
      <input
        name={name} type={type||'text'} value={value||''}
        onChange={onChange} placeholder={placeholder||''}
        style={s.inp} onFocus={inf} onBlur={inb}
      />
    </div>
  )
}
...
function Toggle({ label, desc, checked, onChange }) {
  var bgColor = checked ? 'linear-gradient(135deg,#E8761A,#F5B800)' : 'rgba(184,120,50,.2)'
  var dotLeft = checked ? '23px' : '3px'
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 16px', borderRadius:'12px', background:'#FFF6EA', border:'1.5px solid rgba(232,118,26,.15)' }}>
      ...
      <div onClick={onChange} style={{ width:'46px', height:'26px', borderRadius:'13px', background:bgColor, cursor:'pointer', position:'relative', transition:'background .25s', flexShrink:0 }}>
        <div style={{ position:'absolute', top:'3px', left:dotLeft, width:'20px', height:'20px', borderRadius:'50%', background:'#fff', boxShadow:'0 2px 6px rgba(0,0,0,.15)', transition:'left .25s' }} />
      </div>
    </div>
  )
}
...
export default function SettingsPage() {
  ...
  // ── Load admission settings + site status pages from DB ───────────────────
  useEffect(function() {
    // Load admission settings
    settingsAPI.getAll()
      .then(function(res) {
        var d = res.data || {}
        if (d.admission) setAdmission(function(p) { return {...p, ...d.admission} })
        setLoading(false)
      })
      .catch(function() { setLoading(false) })

    // Load site status pages from DB
  ...
  function chgAdmission(e) {
    var k = e.target.name
    var v = e.target.value
    setAdmission(function(p) { var n = {...p}; n[k] = v; return n })
  }

  function updatePageStatus(id, status) {
    setPages(function(prev) {
      return prev.map(function(p) {
        return p._id === id ? {...p, status} : p
      })
    })
  }
  ...
  async function handleSaveAdmission() {
    ...
    setError('')
    try {
      await settingsAPI.save({ admission })
      setSaved(true)
  ...
  // ── Save site status pages to DB ──────────────────────────────────────────
  async function handleSavePageStatus() {
    setPagesSaving(true)
    setPagesError('')
    try {
      await siteStatusAPI.updateAll({ pages })
      setPagesSaved(true)
      setTimeout(function() { setPagesSaved(false) }, 2500)
    } catch (err) {
      setPagesError(err.message || 'Failed to save page status')
    } finally {
  ...
  async function handlePw() {
    if (!pwForm.current || !pwForm.newPw) {
      setPwMsg('error:Please fill all fields')
      return
    }
  ...
  var saveText = saving ? '⏳ Saving...' : saved ? '✅ Saved!' : '💾 Save & Go Live'

  var pgSaveBg   = pagesSaved ? 'linear-gradient(135deg,#22a35a,#16a34a)' : 'linear-gradient(135deg,#E8761A,#F5B800)'
  var pgSaveText = pagesSaving ? '⏳ Saving...' : pagesSaved ? '✅ Saved!' : '💾 Save Page Status'
  ...
  return (
    <div style={{ maxWidth:'900px' }}>
      <style>{`
        ...
        .dp-site-ctrl  { display:flex; gap:7px; flex-wrap:wrap; margin-bottom:14px; }
        @media (max-width:600px) { .sp-g2 { grid-template-columns:1fr; } }
        @media (max-width:480px) {
          .dp-page-row   { flex-wrap:wrap; gap:6px; }
          .dp-page-left  { min-width:100%; }
          .dp-page-right { width:100%; justify-content:flex-end; }
          .dp-site-ctrl  { flex-direction:column; }
        }
      `}</style>

      <div style={{ marginBottom:'24px' }}>
        <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:'clamp(20px,4vw,26px)', fontWeight:'700', color:'#1C0A00', margin:'0 0 5px' }}>⚙️ Settings</h1>
      ...
      <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', marginBottom:'22px' }}>
        {TABS.map(function(t) {
          var isA      = tab === t
          var tabBg    = isA ? 'linear-gradient(135deg,#E8761A,#F5B800)' : '#FFFFFF'
          var tabColor = isA ? '#fff' : '#7A4010'
          var tabBorder = isA ? '#E8761A' : 'rgba(232,118,26,.2)'
          var tabShadow = isA ? '0 4px 14px rgba(232,118,26,.25)' : 'none'
          return (
            <button key={t} onClick={function() { setTab(t) }}
              style={{ padding:'9px 18px', borderRadius:'10px', border:'1.5px solid', borderColor:tabBorder, background:tabBg, color:tabColor, fontSize:'13px', fontWeight:'700', cursor:'pointer', transition:'all .2s', boxShadow:tabShadow }}
            >
              {t}
            </button>
          )
      ...
      {error && (
        <div style={{ padding:'10px 14px', borderRadius:'10px', background:'rgba(220,38,38,.08)', border:'1.5px solid rgba(220,38,38,.2)', color:'#dc2626', fontSize:'13px', fontWeight:'700', marginBottom:'14px' }}>
          ❌ {error}
        </div>
      )}
        ...
        <div>
          {loading ? (
            <div style={{ textAlign:'center', padding:'40px' }}>
              <div style={{ width:'36px', height:'36px', border:'4px solid rgba(232,118,26,.2)', borderTopColor:'#E8761A', borderRadius:'50%', animation:'spin .8s linear infinite', margin:'0 auto 10px' }} />
              <div style={{ color:'#B87832', fontSize:'13px' }}>Loading settings...</div>
            </div>
          ) : (
            <div style={s.card}>
              <div style={s.sec}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'12px', marginBottom:'16px' }}>
                  <Toggle
                    ...
                    desc="Show 'Admissions Open' banner on website"
                    checked={admission.open}
                    onChange={function() { setAdmission(function(p) { return {...p, open:!p.open} }) }}
                  />
                  <Toggle
                    label="Show Admission Banner"
                    desc="Display CTA banner across all public pages"
                    checked={admission.showBanner}
                    onChange={function() { setAdmission(function(p) { return {...p, showBanner:!p.showBanner} }) }}
                  />
                </div>
                <div className="sp-g2">
                ...
                <div style={{ marginTop:'14px' }}>
                  ...
                  <textarea name="notice" value={admission.notice||''} onChange={chgAdmission} rows={3}
                    style={{ ...s.inp, resize:'vertical' }} onFocus={inf} onBlur={inb} />
                </div>
              </div>
            </div>
          )}
          {!loading && (
            <div style={{ marginTop:'22px', display:'flex', justifyContent:'flex-end' }}>
              <button onClick={handleSaveAdmission} disabled={saving}
                style={{ ...s.btn, background:saveBg, color:'#fff', boxShadow:'0 6px 20px rgba(232,118,26,.3)', padding:'13px 32px', fontSize:'14px', opacity:saving ? 0.7 : 1 }}
              >
                {saveText}
              </button>
            </div>
      ...
      {tab === 'Site Status' && (
        <div style={s.card}>
          <div style={s.secHd}>🌐 Site Pages Status</div>
          ...
          {pagesError && (
            <div style={{ marginBottom:'12px', padding:'10px 14px', borderRadius:'10px', background:'rgba(220,38,38,.08)', border:'1.5px solid rgba(220,38,38,.2)', color:'#dc2626', fontSize:'13px', fontWeight:'700' }}>
              ❌ {pagesError}
            </div>
          )}

          {pages.length === 0 ? (
            <div style={{ textAlign:'center', padding:'30px', color:'#B87832', fontSize:'13px' }}>
              <div style={{ width:'32px', height:'32px', border:'3px solid rgba(232,118,26,.2)', borderTopColor:'#E8761A', borderRadius:'50%', animation:'spin .8s linear infinite', margin:'0 auto 10px' }} />
              Loading pages...
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="dp-page-right">
                      ...
                      <select
                        value={p.status}
                        onChange={function(e) { updatePageStatus(p._id, e.target.value) }}
                      ...
                      <a href={p.path} target="_blank" rel="noreferrer"
                        style={{ width:'26px', height:'26px', borderRadius:'7px', border:'1.5px solid rgba(232,118,26,.2)', background:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px', textDecoration:'none', flexShrink:0 }}
          ...
          <div style={{ marginTop:'16px', display:'flex', justifyContent:'flex-end' }}>
            <button onClick={handleSavePageStatus} disabled={pagesSaving}
              style={{ ...s.btn, background:pgSaveBg, color:'#fff', boxShadow:'0 6px 20px rgba(232,118,26,.3)', padding:'13px 32px', fontSize:'14px', opacity:pagesSaving ? 0.7 : 1 }}
            >
              {pgSaveText}
            </button>
          </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'14px', maxWidth:'420px' }}>
              ...
              <div>
                ...
                <input type="password" value={pwForm.current}
                  onChange={function(e){ var v=e.target.value; setPwForm(function(p){return{...p,current:v}}) }}
                  style={s.inp} onFocus={inf} onBlur={inb} />
              </div>
              <div>
                <label style={s.label}>New Password</label>
                <input type="password" value={pwForm.newPw}
                  onChange={function(e){ var v=e.target.value; setPwForm(function(p){return{...p,newPw:v}}) }}
                  style={s.inp} onFocus={inf} onBlur={inb} />
              </div>
              <div>
                <label style={s.label}>Confirm New Password</label>
                <input type="password" value={pwForm.confirm}
                  onChange={function(e){ var v=e.target.value; setPwForm(function(p){return{...p,confirm:v}}) }}
                  style={s.inp} onFocus={inf} onBlur={inb} />
              </div>
              <button onClick={handlePw}
                style={{ ...s.btn, background:'linear-gradient(135deg,#E8761A,#F5B800)', color:'#fff', boxShadow:'0 4px 14px rgba(232,118,26,.3)', alignSelf:'flex-start', padding:'12px 24px' }}>
                🔐 Update Password
              </button>
            </div>
          </div>
