...
var FILE_ICONS = { pdf:'📄', doc:'📝', xls:'📊', img:'🖼️', zip:'🗜️' }
  ...
  return (
    <div style={{maxWidth:'1100px'}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'24px',gap:'16px',flexWrap:'wrap'}}>
        <div>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'26px',fontWeight:'700',color:'#1C0A00',margin:'0 0 5px'}}>⬇️ Downloads Management</h1>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'20px'}}>
        {[{label:'Total Documents',value:docs.length,icon:'📁',clr:'#E8761A'},{label:'Total Downloads',value:totalDl,icon:'⬇️',clr:'#6C3FC5'},{label:'Active',value:docs.filter(function(d){return d.status==='Active'}).length,icon:'✅',clr:'#22a35a'},{label:'Categories',value:CATS.length-1,icon:'🏷️',clr:'#C45F0A'}].map(function(st){
          return (
      ...
      <div style={{...s.card,marginBottom:'16px',padding:'16px 20px'}}>
        <div style={{display:'flex',gap:'10px',flexWrap:'wrap',alignItems:'center'}}>
          <input value={search} onChange={function(e){setSearch(e.target.value)}} placeholder="🔍  Search documents..." style={{...s.inp,width:'220px',padding:'9px 13px'}} onFocus={inf} onBlur={inb} />
                  ...
                  <tr key={d.id} style={{borderBottom:'1px solid rgba(232,118,26,.07)',background:i%2===0?'#FFFFFF':'#FFFDF8',transition:'background .15s'}}
                    ...
                    <td style={{padding:'12px 14px',maxWidth:'260px'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                        <span style={{fontSize:'20px'}}>{FILE_ICONS[d.type]||'📄'}</span>
                    ...
                    <td style={{padding:'12px 14px'}}>
                      <button onClick={function(){toggleStatus(d.id)}} style={{padding:'4px 11px',borderRadius:'20px',fontSize:'11px',fontWeight:'800',border:'none',cursor:'pointer',background:d.status==='Active'?'rgba(34,163,90,.12)':'rgba(196,95,10,.12)',color:d.status==='Active'?'#22a35a':'#C45F0A',transition:'all .2s'}}>
                        {d.status==='Active'?'✅ Active':'🙈 Hidden'}
                      </button>
                    </td>
                    <td style={{padding:'12px 14px'}}>
                      <div style={{display:'flex',gap:'6px'}}>
                        <button onClick={function(){openEdit(d)}} style={{width:'32px',height:'32px',borderRadius:'8px',border:'1.5px solid rgba(232,118,26,.2)',background:'#FFF6EA',cursor:'pointer',fontSize:'14px',display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}
                        ...
                        <button onClick={function(){openDel(d.id)}} style={{width:'32px',height:'32px',borderRadius:'8px',border:'1.5px solid rgba(220,38,38,.15)',background:'rgba(254,242,242,.6)',cursor:'pointer',fontSize:'14px',display:'flex',alignItems:'center',justifyContent:'center',transition:'all .15s'}}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(28,10,0,.4)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:'20px'}} onClick={function(e){if(e.target===e.currentTarget)closeModal()}}>
          <div style={{background:'#FFFDF8',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.2)',padding:'30px',width:'100%',maxWidth:'520px',maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(28,10,0,.2)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'22px'}}>
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:'20px',fontWeight:'700',color:'#1C0A00',margin:0}}>{modal==='add'?'+ Upload Document':'✏️ Edit Document'}</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'14px'}}>
              <div>
                ...
                <input name="title" value={current.title} onChange={handleChange} placeholder="Document name..." style={s.inp} onFocus={inf} onBlur={inb} />
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                ...
                <div>
                  ...
                  <select name="type" value={current.type} onChange={handleChange} style={s.inp} onFocus={inf} onBlur={inb}>
                    {FILE_TYPES.map(function(t){ return <option key={t}>{t}</option> })}
                  </select>
                </div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
                <div>
                  ...
                  <input name="size" value={current.size} onChange={handleChange} placeholder="e.g. 245 KB" style={s.inp} onFocus={inf} onBlur={inb} />
                </div>
                <div>
