    ...
    >
      <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px'}}>
        ...
        <div style={{display:'flex', gap:'16px', alignItems:'center'}}>
          <div style={{width:'50px', height:'50px', borderRadius:'14px', background: isOpen ? 'rgba(232,118,26,.12)' : 'var(--bg2)', border:'1.5px solid '+(isOpen ? 'rgba(232,118,26,.25)' : 'var(--brd)'), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'22px', flexShrink:0}}>
            {isOpen ? '💼' : '✅'}
          </div>
          <div>
        ...
        <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
          <span style={{fontSize:'11px', fontWeight:'800', padding:'4px 12px', borderRadius:'50px', background: isOpen ? 'rgba(34,163,90,.12)' : 'rgba(0,0,0,.06)', color: isOpen ? '#22a35a' : 'var(--txt3)'}}>
            {isOpen ? '● Open' : '✓ Filled'}
          </span>
          {isOpen && (
            <button
