...
export default function SchoolHistory() {
  ...
  return (
    <section id="history" style={{padding:'90px 0',background:'linear-gradient(180deg,#FFFDF8 0%,#FFF6EA 50%,#FFFDF8 100%)',position:'relative',overflow:'hidden'}}>
          ...
          {MILESTONES.map(function(m,i){
            ...
            return (
              <div key={i} ref={function(el){refs.current[i]=el}} data-idx={i}
                style={{display:'flex',justifyContent:isLeft?'flex-start':'flex-end',marginBottom:'52px',position:'relative',
                  opacity:isVis?1:0,
                ...
                <div style={{position:'absolute',left:'50%',top:'20px',transform:'translateX(-50%)',zIndex:3}}>
                  <div style={{width:'50px',height:'50px',borderRadius:'50%',background:'linear-gradient(135deg,#E8761A,#F5B800)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'22px',boxShadow:'0 0 0 4px #FFF6EA, 0 0 0 7px rgba(232,118,26,.18), 0 8px 24px rgba(232,118,26,.3)',transition:'transform .25s'}}
                ...
                <div style={{width:'43%',background:'#FFFFFF',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.1)',padding:'24px 26px',boxShadow:'0 4px 20px rgba(232,118,26,.07)',transition:'all .28s'}}
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'9px'}}>
                    <span style={{fontFamily:"'Playfair Display',serif",fontSize:'27px',fontWeight:'700',background:'linear-gradient(135deg,#E8761A,#F5B800)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>{m.year}</span>
                    <div style={{height:'2px',flex:1,background:'linear-gradient(90deg,#E8761A,transparent)',borderRadius:'2px'}} />
                  </div>
                  <div style={{fontSize:'15px',fontWeight:'700',color:'#1C0A00',marginBottom:'8px'}}>{m.title}</div>
                  <div style={{fontSize:'13.5px',color:'#7A4010',lineHeight:'1.72'}}>{m.desc}</div>
                </div>
              </div>
            )
          })}
