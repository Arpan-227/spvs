...
export default function DirectorMessage() {
  ...
  return (
    <section id="director" ref={ref} style={{padding:'90px 0',background:'linear-gradient(135deg,#1C0A00 0%,#3D1A00 60%,#1C0A00 100%)',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{maxWidth:'1100px',margin:'0 auto',padding:'0 24px',position:'relative',zIndex:1}}>
        ...
        <div style={{textAlign:'center',marginBottom:'56px',opacity:vis?1:0,transform:vis?'none':'translateY(24px)',transition:'all .65s ease'}}>
          <div className="chip" style={{background:'rgba(245,184,0,.1)',borderColor:'rgba(245,184,0,.25)',color:'#F5B800'}}><span style={{width:'6px',height:'6px',borderRadius:'50%',background:'#F5B800',animation:'blink 2s infinite',flexShrink:0,display:'inline-block'}} />Leadership</div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:'52px',alignItems:'center'}}>

          {/* Avatar */}
          <div style={{textAlign:'center',opacity:vis?1:0,transform:vis?'none':'translateX(-30px)',transition:'all .7s ease .15s'}}>
            <div style={{width:'170px',height:'170px',borderRadius:'50%',background:'linear-gradient(135deg,#E8761A,#F5B800)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'68px',margin:'0 auto 20px',boxShadow:'0 0 0 6px rgba(232,118,26,.15), 0 0 0 12px rgba(232,118,26,.07), 0 20px 60px rgba(232,118,26,.3)',animation:'pulseRing 3s ease-in-out infinite'}}>👔</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:'19px',fontWeight:'700',color:'#FFCF40',marginBottom:'5px'}}>Sh. Awadhesh Narayan Agarwal</div>
            <div style={{fontSize:'11.5px',fontWeight:'800',color:'rgba(245,184,0,.5)',letterSpacing:'1.5px',textTransform:'uppercase',marginBottom:'16px'}}>Director & Manager</div>
          ...
          <div style={{opacity:vis?1:0,transform:vis?'none':'translateX(30px)',transition:'all .7s ease .25s'}}>
            <div style={{fontSize:'72px',color:'rgba(245,184,0,.1)',fontFamily:'Georgia,serif',lineHeight:.8,marginBottom:'-8px',userSelect:'none'}}>"</div>
            <div style={{display:'flex',flexDirection:'column',gap:'15px'}}>
              {[
                ...
                return <p key={i} style={{fontSize:'15px',color:'rgba(255,220,150,.75)',lineHeight:'1.8',margin:0}} dangerouslySetInnerHTML={{__html:para.replace(/<b>/g,'<strong style="color:#F5B800">').replace(/<\/b>/g,'</strong>')}} />
              })}
            </div>
            <div style={{marginTop:'28px',padding:'18px 22px',borderRadius:'16px',background:'rgba(232,118,26,.1)',border:'1px solid rgba(232,118,26,.22)',display:'inline-block'}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:'15px',fontStyle:'italic',color:'#FFCF40',marginBottom:'5px'}}>"Great things are done by a series of small things brought together."</div>
