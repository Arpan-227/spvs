...
export default function SchoolValues() {
  ...
  return (
    <section id="values" ref={ref} style={{padding:'90px 0',background:'#FFFDF8',position:'relative',overflow:'hidden'}}>
      ...
      <div style={{maxWidth:'1100px',margin:'0 auto',padding:'0 24px',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:'16px',marginBottom:'64px'}}>
          {VALUES.map(function(v,i){
            return (
              <div key={i} style={{background:'#FFFFFF',borderRadius:'20px',border:'1.5px solid rgba(232,118,26,.08)',padding:'28px',boxShadow:'0 4px 16px rgba(232,118,26,.05)',display:'flex',gap:'18px',alignItems:'flex-start',transition:'all .28s',cursor:'default',opacity:vis?1:0,transform:vis?'none':'translateY(20px)',transitionDelay:(.1+i*.08)+'s'}}
                ...
                <div style={{width:'54px',height:'54px',borderRadius:'15px',background:v.color+'14',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'28px',flexShrink:0,border:'1.5px solid '+v.color+'20',transition:'transform .25s'}}
                  onMouseEnter={function(e){e.currentTarget.style.transform='scale(1.12)'}}
                  onMouseLeave={function(e){e.currentTarget.style.transform='scale(1)'}}>{v.icon}</div>
                <div>
        ...
        <div ref={statsRef} style={{background:'linear-gradient(135deg,#1C0A00 0%,#3D1A00 50%,#1C0A00 100%)',borderRadius:'28px',padding:'52px 40px',display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'24px',position:'relative',overflow:'hidden',boxShadow:'0 24px 64px rgba(28,10,0,.2)'}}>
          ...
          <div style={{position:'absolute',top:0,left:0,right:0,height:'3px',background:'linear-gradient(90deg,transparent,#E8761A,#F5B800,#E8761A,transparent)',borderRadius:'28px 28px 0 0'}} />
          {STATS.map(function(st,i){
            return (
              <div key={i} style={{textAlign:'center',position:'relative',zIndex:1,opacity:statsVis?1:0,transform:statsVis?'none':'translateY(20px)',transition:'all .6s ease '+(i*.1)+'s'}}>
                <div style={{fontSize:'34px',marginBottom:'10px'}}>{st.icon}</div>
                <div style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(30px,3.5vw,44px)',fontWeight:'700',color:'#FFCF40',marginBottom:'6px',lineHeight:1}}>
                  <CountUp target={st.n} started={statsVis} />{st.sfx}
                </div>
                <div style={{fontSize:'12.5px',color:'rgba(255,220,150,.55)',fontWeight:'600',letterSpacing:'.5px'}}>{st.label}</div>
              </div>
            )
          })}
        </div>
